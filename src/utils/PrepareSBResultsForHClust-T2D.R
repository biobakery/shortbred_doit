...
option_list <- list(
  make_option("--all_results",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\WGSResults\\AllResults+ABRClass.tab", type="character",
...
# Output
#############################
  make_option("--out_results",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\DetailedClean.tab", type="character",
              help="Enter the file with the Yatsunenko metadata."),
  make_option("--out_classresults",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\ABRClassesClean.tab", type="character",
              help="Enter the file with the Yatsunenko metadata."),
  make_option("--out_meta",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\MetaClean.tab", type="character",
              help="Enter the file with the Yatsunenko metadata."),
  make_option("--out_meta_gender",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\MetaCleanGender.tab", type="character",
            help="Enter the file with the Yatsunenko metadata."),
  make_option("--out_meta_ds",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\MetaCleanDS.tab", type="character",
            help="Enter the file with the Yatsunenko metadata."),
make_option("--out_meta_country",default="C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check\\MetaCleanCountry.tab", type="character",
            help="Enter the file with the Yatsunenko metadata.")
...
dfOtherMet <- rbind(dfHMPMeta,dfYATMeta)
matZero <- matrix(0,nrow=dim(dfOtherMet)[1],ncol=dim(dfMeta)[2]-dim(dfOtherMet)[2])
dfOtherMet <- cbind(dfOtherMet,matZero)
dfOtherMet[c(3,4)] <- NA
colnames(dfOtherMet) <-colnames(dfMeta)
dfMeta <- rbind(dfMeta,dfOtherMet)

setwd("C:\\Users\\Jim\\Dropbox\\ShortBRED\\results\\T2D-Check")

for (i in c(2:dim(dfMeta)[2])){
  strName <- paste0("Meta",i,".tab")
  write.table(dfMeta[c(1,i)],strName,sep="\t",col.names=T,row.names=F)
}
