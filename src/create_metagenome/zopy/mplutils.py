...
def funcAddLegend( axes, elements, start=0.9, spacing=0.1, margin=0.1, textbuffer=0.1 ):
    """ 
    ...
    for ( marker, color, edgecolor, size, label ) in elements:
        ...
        if match:
            axes.text( margin, height, match.group( 1 ), va="center", ha="center", size="8", color=color, weight="bold" )
        else:
            axes.scatter( [margin], [height], marker=marker, s=size, color=color, edgecolor=edgecolor )
        axes.text( margin + textbuffer, height, label, va="center", ha="left", size=8, clip_on=False )
        height -= spacing
