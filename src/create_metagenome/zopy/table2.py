...
class table:
    ...
    def __init__ ( self, source=None, skip_remap=False, verbose=True ):
        ...
        if isinstance( source, list ):
            ...
            with open( source ) if isinstance( source, str ) else sys.stdin as fh:
    ...
    def rowsort( self ):
        ...
        self.data = [self.data[0]] + [self.data[self.rowmap[rowhead]] for rowhead in sorted( self.rowheads )]
        self.remap()
    ...
    def grep ( self, index, patterns, **kwargs ):
        ...
        if isinstance( patterns, str ):
            patterns = [patterns]
        self.report( "applying grep", "index=", index, "patterns=", patterns, kwargs )
    ...
    def select ( self, index, choices, **kwargs ):
        ...
        if isinstance( choices, str ):
            choices = [choices]
        self.report( "applying select", "index=", index, "choices=", 
                    choices if len( choices ) < c_iMaxPrintChoices else "< # choices = %d >" % ( len( choices ) ), kwargs )
    ...
    def delete ( self, index, choices, **kwargs ):
        ...
        if isinstance( choices, str ):
            choices = [choices]
        # this allows us to test if choices is a scalar or list; strange because strings are iterable?
        self.report( "applying delete", "index=", index, "choices=", 
                    choices if len( choices ) < c_iMaxPrintChoices else "< # choices = %d >" % ( len( choices ) ), kwargs )
...
def nesteddict2table ( d, aRowheads=None, aColheads=None, empty=c_strNA ):
    ...
    aaData = [ [c_strHeaders] + aColheads ]
    for rowhead in aRowheads:
        aRow = [ rowhead ] + [ d[rowhead][colhead] if colhead in d[rowhead] else empty for colhead in aColheads ]
        aaData.append( aRow )
    return table( aaData )
